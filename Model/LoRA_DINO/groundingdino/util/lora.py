...
def add_lora_to_model(model, rank=128, inference=False):
    """
    ...
    config = LoraConfig(
        ...
        lora_alpha=rank,
        target_modules=[
             # ---------- Encoder MS-Deformable Attention (per-layer) ----------
            "self_attn.sampling_offsets",   # model.transformer.encoder.layers.X.self_attn.sampling_offsets
            ...
            "encoder.layers.0.linear1", "encoder.layers.0.linear2",
            "encoder.layers.1.linear1", "encoder.layers.1.linear2",
            "encoder.layers.2.linear1", "encoder.layers.2.linear2",
            "encoder.layers.3.linear1", "encoder.layers.3.linear2",
            "encoder.layers.4.linear1", "encoder.layers.4.linear2",
            "encoder.layers.5.linear1", "encoder.layers.5.linear2",

             # BERT last 2 layers (reshape class tokens slightly)
