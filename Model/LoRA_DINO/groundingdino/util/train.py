...
def load_model(model_config_path: str, model_checkpoint_path: str, device: str = "cuda", use_lora: bool = False):
    ...
    # Build full model (including BERT from get_tokenlizer)
    model = build_model(args)

    # Load SWIN + decoder weights (model_checkpoint_path)
    checkpoint = torch.load(model_checkpoint_path, map_location="cpu")
    state = clean_state_dict(checkpoint["model"])

    # IMPORTANT: DO NOT REMOVE BERT WEIGHTS (your custom BERT has correct vocab already)
