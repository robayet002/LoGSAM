...
# --- added imports for JSON saving (non-breaking) ---
import json
    ...
    def visualize_epoch(self, model, val_loader, epoch, prepare_data, box_th=0.3,txt_th=0.2):
        ...
        with torch.no_grad():
            for idx, batch in enumerate(val_loader):
                ...
                cv2.imwrite(f"{save_dir}/val_pred_{idx}", img_bgr)
    ...
    def visualize_image(self, model, image, caption,image_source,fname,device="cuda", box_th=0.3,txt_th=0.2):
        ...
        with torch.no_grad():          
            ...
            if len(filtered_boxes):
                ...
                # --- added: write one JSON per image (non-breaking) ---
                try:
                    out_json = Path(self.save_dir) / "preds" / (Path(fname).stem + ".json")
                    
                    data = {
                        "image_name": fname,
                        "labels": phrases,  # human-readable phrases
                        "scores": filtered_logits.max(dim=1)[0].numpy().astype(float).tolist(),
                        "bboxes": [[float(x) for x in bb] for bb in xyxy.tolist()]  # absolute xyxy
                    }
                    with open(out_json, "w", encoding="utf-8") as f:
                        json.dump(data, f, indent=2)
                except Exception as e:
                    print(f"[warn] failed to save JSON for {fname}: {e}")
            else:
